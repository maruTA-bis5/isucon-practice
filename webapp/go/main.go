...
func initialize(c echo.Context) error {
	...
	for _, p := range paths {
		...
		if out, err := exec.Command("bash", "-c", cmdStr).Output(); err != nil {
			c.Logger().Errorf("Initialize script error : %v", err)
			c.Logger().Errorf("output: %v", out)
			c.Logger().Errorf("file: %v", sqlFile)
...
func searchChairs(c echo.Context) error {
	...
	conditions = append(conditions, "has_stock")
	...
	err = dbGet(db, c, &res.Count, countQuery+searchCondition, params...)
	if err != nil {
...
func buyChair(c echo.Context) error {
	...
	var chair Chair
	err = txQueryRowx(tx, c, "SELECT * FROM chair WHERE id = ? AND has_stock FOR UPDATE", id).StructScan(&chair)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	_, err = txExec(tx.Tx, c, "UPDATE chair SET stock = stock - 1 WHERE id = ?", id)
	if err != nil {
...
func getLowPricedChair(c echo.Context) error {
	var chairs []Chair
	query := `SELECT * FROM chair WHERE has_stock ORDER BY price ASC, id ASC LIMIT ?`
...
func getEstateDetail(c echo.Context) error {
	...
	var estate Estate
	err = dbGet(db.Unsafe(), c, &estate, "SELECT * FROM estate WHERE id = ?", id)
	if err != nil {
		if err == sql.ErrNoRows {
...
func postEstate(c echo.Context) error {
	...
	for _, row := range records {
		...
		_, err := txExec(tx, c, "INSERT INTO estate(id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features, popularity) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)", id, name, description, thumbnail, address, latitude, longitude, rent, doorHeight, doorWidth, features, popularity)
		if err != nil {
...
func searchEstates(c echo.Context) error {
	...
	estates := []Estate{}
	params = append(params, perPage, page*perPage)
	err = dbSelect(db.Unsafe(), c, &estates, searchQuery+searchCondition+limitOffset, params...)
	if err != nil {
		if err == sql.ErrNoRows {
...
func getLowPricedEstate(c echo.Context) error {
	...
	err := dbSelect(db.Unsafe(), c, &estates, query, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	err = dbSelect(db.Unsafe(), c, &estates, query, w, h, w, d, h, w, h, d, d, w, d, h, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
			return c.JSON(http.StatusOK, EstateListResponse{[]Estate{}})
		}
...
func searchEstateNazotte(c echo.Context) error {
	...
	query := fmt.Sprintf(`SELECT * FROM estate WHERE latitude <= ? AND latitude >= ? AND longitude <= ? AND longitude >= ? AND ST_Contains(ST_PolygonFromText(%s), location) ORDER BY popularity DESC, id ASC`, coordinates.coordinatesToText())
	err = dbSelect(db.Unsafe(), c, &estatesInPolygon, query, b.BottomRightCorner.Latitude, b.TopLeftCorner.Latitude, b.BottomRightCorner.Longitude, b.TopLeftCorner.Longitude)
	if err == sql.ErrNoRows {
...
func postEstateRequestDocument(c echo.Context) error {
	...
	err = dbGet(db.Unsafe(), c, &estate, query, id)
	if err != nil {
		if err == sql.ErrNoRows {
			return c.NoContent(http.StatusNotFound)
		}
